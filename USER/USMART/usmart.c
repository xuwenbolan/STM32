...
void usmart_reset_runtime(void)
{
	...
	LL_TIM_SetAutoReload(TIM4,0XFFFF);//将重装载值设置到最大
	LL_TIM_SetCounter(TIM4,0);		//清空定时器的CNT
...
{
	if(LL_TIM_IsActiveFlag_UPDATE(TIM4)==SET)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=LL_TIM_GetCounterMode(TIM4);
...
{ 		   
//LL_TIM_GetITStatus();
//	LL_TIM_IsITFlag_UPDATE();
	if(LL_TIM_IsActiveFlag_UPDATE(TIM4)==SET)//溢出中断
	{
//		printf("tim runing\r\n");
		usmart_dev.scan();	//执行usmart扫描	
		LL_TIM_SetCounter(TIM4,0);		//清空定时器的CNT
		LL_TIM_SetAutoReload(TIM4,100);//恢复原来的设置		    
	  LL_TIM_ClearFlag_UPDATE(TIM4);  //清除中断标志位    		
	}	
...
{
#if USMART_ENTIMX_SCAN==1
	Timer4_Init(1000,(u32)(sysclk*1000));//分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
